...
5. Bind `ConversationMemberServiceContract` to your own service implementation in your application's service provider:

```php
use RonasIT\Chat\Contracts\Services\ConversationMemberServiceContract;

$this->app->bind(ConversationMemberServiceContract::class, YourConversationMemberService::class);
```

   Your implementation must provide a `getByList(array $ids)` method that returns a collection of user models matching the given IDs. This is used to validate `member_ids` when updating a conversation.

6. All routes are registered by default, you can change the route registration by calling `Route::chat()` in your routes file (e.g. `routes/api.php`).
